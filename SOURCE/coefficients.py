...
from functions import moldata_read,averages_read,prediction2coefficients
...
for itest,imol in enumerate(testrange):
    rho = prediction2coefficients(atomic_numbers[imol], lmax, nmax, coeff[itest], av_coefs)
    np.savetxt(outfilebase+str(imol)+'.dat', rho)
