...
for frac in fracs:
  ...
  outfilebase1 = outfilebase+"tf"+str(frac)+"_"
  ...
  for itest,imol in enumerate(test_configs):
    ...
    np.savetxt(outfilebase1+'gpr_'  +str(imol)+'.dat', rho)
    np.savetxt(outfilebase1+'pyscf_'+str(imol)+'.dat', rho1)
    ...
    if use_charges:
      ...
      np.savetxt(outfilebase1+'gpr_'  +str(imol)+'.N.dat', rho_n)
      np.savetxt(outfilebase1+'pyscf_'+str(imol)+'.N.dat', rho_n1)

      rho_u = rho + q * (N-q@rho) / (q@q)
      rho_u1 = gpr2pyscf(atoms, lmax, nmax, rho_u)
      np.savetxt(outfilebase1+'gpr_'  +str(imol)+'.unit.dat', rho_u)
      np.savetxt(outfilebase1+'pyscf_'+str(imol)+'.unit.dat', rho_u1)

      rho_s = rho * N / (q@rho)
      rho_s1 = gpr2pyscf(atoms, lmax, nmax, rho_s)
      np.savetxt(outfilebase1+'gpr_'  +str(imol)+'.scaled.dat', rho_s)
      np.savetxt(outfilebase1+'pyscf_'+str(imol)+'.scaled.dat', rho_s1)
