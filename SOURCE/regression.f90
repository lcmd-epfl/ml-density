SUBROUTINE getab(path_proj,path_over,path_kern,trrange,atomspe,llmax,nnmax,nspecies,ntrain,M,natmax,natoms,totsize,&
...
CHARACTER*512:: path_proj,path_over,path_kern
...
!f2py intent(in) path_proj,path_over,path_kern,trrange,atomspe,llmax,nnmax,nspecies,ntrain,M,natmax,natoms
...
!$OMP FIRSTPRIVATE(path_proj,path_over,path_kern) &
...
do itrain=1,ntrain
   ...
   if2 = itrain + 2000
   if3 = itrain + 4000
   open(unit=if1,file=trim(path_proj)//trim(adjustl(conf_str))//'.dat',action='read',status='old')
   open(unit=if2,file=trim(path_over)//trim(adjustl(conf_str))//'.dat',action='read',status='old')
   it1 = 1
   ...
   close(if2)
   close(if1)
   open(unit=if3,file=trim(path_kern)//trim(adjustl(conf_str))//'.dat',action='read',status='old')
   ik1 = 1
   do iref1=1,M
