...
#pragma omp parallel for schedule(dynamic)
  for(int itest=0; itest<ntest; itest++){
    ...
    int * kernsparseindices = kernsparseindices_fill(nat, llmax, M, atomcount[itest], ref_elem , alnum);
    ...
    for(int iref=0; iref<M; iref++){
      ...
      for(int l=0; l<al; l++){
        ...
        for(int icel=0; icel<atomcount[itest][a]; icel++){
          ...
          int sk = kernsparseindices[KSPARSEIND(iref,l,icel)];
          for(int n=0; n<anc; n++){
            for(int imm=0; imm<msize; imm++){
              double d = 0.0;
              for(int im=0; im<msize; im++){
                d += kernels[sk+im*msize+imm] * w[iref][l][n][im];
              }
