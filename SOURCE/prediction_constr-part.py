...
for frac_base in [fracs[0]]:
  for frac in fracs:
    ...
    if not training:
      ...
      predictfile = predictfilebase + "_trainfrac"+str(frac_base)+"_"+str(frac)+"_M"+str(M)+"_reg"+str(reg)+"_jit"+str(jit)+".npy"
    else:
      ntest,test_configs = get_training_set(trainfilename, frac_base)
      predictfile = predictfilebase + "_training_trainfrac"+str(frac_base)+"_"+str(frac)+"_M"+str(M)+"_reg"+str(reg)+"_jit"+str(jit)+".npy"
